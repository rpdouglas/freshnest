...
# ====================================================
# FRESH NEST: REVENUE DASHBOARD INSTALLER
# Feature: Analytics & Reporting
# Approach: Client-Side Aggregation + Recharts
# ====================================================

echo "🚀 Installing Revenue Dashboard..."
...
# This is the "Brain" that processes raw job data into charts
...
# 3. Create Revenue Chart Component (Responsive)
...
# 4. Create Staff Dashboard (Role Specific View)
            ...
            <div key={job.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-start gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-1 text-sm text-slate-500 mt-1">
                  <MapPin size={14} />
                  Client ID: {job.clientId.slice(0, 5)}...
                </div>
                <div className="mt-2">
...
# 5. Create Admin Dashboard (The Main View)
...
# 6. Create the Main Dashboard Page Container
...
const DashboardPage = () => {
  const { stats, role, loading, error } = useDashboard();
  ...
  return role === 'admin' ? <AdminDashboard stats={stats} /> : <StaffDashboard jobs={stats.recentActivity} />;
};

export default DashboardPage;
INNER_EOF

# 7. Update App.jsx to use the new Page
# Replacing the placeholder Dashboard component
...
echo "✅ SUCCESS! Revenue Dashboard installed."
